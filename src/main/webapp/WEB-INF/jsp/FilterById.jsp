...
<form action="Search/${id}" method="get">
	<p>Ingresar ID a buscar:</p>
	<input type="number" class="form-control" id="id" placeholder="ingrese id">
...
<c:if test="${not empty sillones}">
	...
	<table class="table table-bordered table-dark">
		...
		<tbody>
			<tr>
				<td>${sillones.id}</td>
				<c:if test="${sillones.estado == true}">
					<td>Disponible</td>
				</c:if>
				<c:if test="${sillones.estado == false}">
					<td>Ocupado</td>
				</c:if>
	        </tr>
		</tbody>
